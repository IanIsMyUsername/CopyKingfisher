...
extension KingfisherWrapper where Base == CGSize {
    ...
    public func constrained(_ size: CGSize) -> CGSize {
        let aspectWidth = round(aspectRadio * size.height)
        let aspectHeight = round(size.width / aspectRadio)
    ...
    public func filling(_ size: CGSize) -> CGSize {
        let aspectWidth = round(aspectRadio * size.height)
        let aspectHight = round(size.width / aspectRadio)
        
        return aspectWidth < size.width ?
            CGSize(width: size.width, height: aspectHight) :
            CGSize(width: aspectWidth, height: size.height)
    }
    ...
    private var aspectRadio: CGFloat {
