    ...
    public class Backend<T: DataTransformable> {
            ...
            expiration: StorageExpiration? = nil) throws
        {
            ...
            do {
                ...
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.cannotConvertToData(object: value, error: error))
            }
            ...
            do {
                ...
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.cannotCreateCacheFile(fileURL: fileURL, key: key, data: data, error: error)
                )
            }
            ...
            let attributes: [FileAttributeKey : Any] = [
                ...
                FileAttributeKey.creationDate: now.fileAtrributeDate,
                // The estimated expiration date.
                FileAttributeKey.modificationDate: expiration.estimatedExpirationSinceNow.fileAtrributeDate
            ]
            do {
                try config.fileManager.setAttributes(attributes, ofItemAtPath: fileURL.path)
            } catch {
                try? config.fileManager.removeItem(at: fileURL)
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.cannotSetCacheFileAttribute(filePath: fileURL.path, attributes: attributes, error: error
                    )
                )
            }
        }
        ...
        func value(forKey key: String, extendingExpiration: ExpirationExtending = .cacheTime) throws -> T? {
            return try value(forKey: key, referenceDate: Date(), actruallyLoad: true, extendingExpiration: extendingExpiration)
        }
            ...
            extendingExpiration: ExpirationExtending) throws -> T?
        {
            ...
            let meta: FileMeta
            do {
                let resourceKeys: Set<URLResourceKey> = [URLResourceKey.contentAccessDateKey, URLResourceKey.creationDateKey]
                meta = try FileMeta(fileURL: fileURL, resourceKeys: resourceKeys)
            } catch {
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.invalidURLResource(error: error, key: key, url: fileURL))
            }
            ...
            if !actruallyLoad { return T.empty }
            ...
            do {
                ...
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.cannotLoadDataFromDisk(url: fileURL, error: error))
            }
        }
        ...
        func isCached(forKey key: String, referenceDate: Date) -> Bool {
            do {
                let result = try value(forKey: key, referenceDate: referenceDate, actruallyLoad: false, extendingExpiration: .none)
                return result != nil
            } catch {
                return false
            }
        ...
        func allFileURLs(for propertyKeys: [URLResourceKey]) throws -> [URL] {
            ...
            guard let directoryEnumerator = fileManager.enumerator(at: directoryURL, includingPropertiesForKeys: propertyKeys, options: .skipsHiddenFiles) else {
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.fileEnumeratorCreationFailed(url: directoryURL))
            }
            guard let urls = directoryEnumerator.allObjects as? [URL] else {
                throw KingfisherError.cacheError(reason: KingfisherError.CacheErrorReason.invalidFileEnumeratorContent(url: directoryURL))
            }
            return urls
        }
...
extension DiskStorage {
    struct FileMeta {
        ...
        static func lastAccessDate(lhs: FileMeta, rhs: FileMeta) -> Bool {
            return lhs.lastAccessDate ?? Date.distantPast > rhs.lastAccessDate ?? Date.distantPast
        }
        ...
        func extendExpiration(with fileManager: FileManager, extendingExpiration: ExpirationExtending) {
            ...
            case .none:
                // not extending expitation time here
                return
            case .cacheTime:
                let originalExpiration: StorageExpiration =
                    .seconds(lastEstimatedExpiration.timeIntervalSince(lastAccessDate))
                attributes = [
                    FileAttributeKey.creationDate: Date().fileAtrributeDate,
                    FileAttributeKey.modificationDate:
                        originalExpiration.estimatedExpirationSinceNow.fileAtrributeDate
                ]
            case .expirationTime(let expirationTime):
                attributes = [
                    FileAttributeKey.creationDate: Date().fileAtrributeDate,
                    FileAttributeKey.modificationDate:
                        expirationTime.estimatedExpirationSinceNow.fileAtrributeDate
                ]
            }
