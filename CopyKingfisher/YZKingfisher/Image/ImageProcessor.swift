...
public struct BlendImageProcessor: ImageProcessor {
    ...
    public func process(item: ImageProcessItem, options: KingfisherParsedOptionsInfo) -> KFCrossPlatformImage? {
        switch item {
            case .image(let image):
                return image.kf.scaled(to: options.scaleFactor).kf.image(withBlendMode: blendMode, alpha: alpha, backgroundColor: backgroundColor)
...
public struct BlackWhiteProcessor: ImageProcessor {
    ...
    /// Identifier of the processor.
    /// - Note: See documentation of `ImageProcessor` protocol for more.
    public var identifier: String = "com.onevcat.Kingfisher.BlackWhiteProcessor"
